...
btc_data = []
for c in xpoints:
    btc_data.append(data["vm_to_vm_1"]["c=%d"%c]["rx.rate_MBps"]["sum"])

lines.append(ax.plot(xpoints, btc_data, "o-"))

btc_data = []
for c in xpoints:
    btc_data.append(data["vm_to_vm_2"]["c=%d"%c]["rx.rate_MBps"]["sum"])

lines.append(ax.plot(xpoints, btc_data, "+-"))

btc_data = []
for c in xpoints:
    btc_data.append(data["vm_to_vm_3"]["c=%d"%c]["rx.rate_MBps"]["sum"])

lines.append(ax.plot(xpoints, btc_data, "*-"))

btc_data = []
for c in xpoints:
    btc_data.append(data["vm_to_vm_4"]["c=%d"%c]["rx.rate_MBps"]["sum"])
